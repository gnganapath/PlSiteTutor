...
AdminTestHelper =
  login: (done) ->
    ...
    request.post options, (err, _response, _body) ->
      # HACK until request module followAllRedirects works again
      request.get _response.headers.location, ->
        done()
  dropDB: (callback) ->
          ...
          AdminTestHelper.dropDB ->
            options =
              ...
              form:
                ...
                type: 'sweet'
              followAllRedirects: false
            request.post options, (ignoreErr, response, postResponseBody) ->
              throw new Error(postResponseBody) if response.statusCode >= 400
              redirectURL = testutils.absoluteURLForPath response.headers.location
              # HACK until followAllRedirects is fixed with request module and Node 0.8
